...
def main(
    ...
    synthetic_valid_dataset_path: str = "datasets/synthetic_valid_dataset",
    checkpoints_path: str = "checkpoints",
    log_n_predictions: int = 36,
) -> None:
    ...
    synthetic_valid_dataset = dm_codes.datasets.DMMapDatasetFromFolder(folder=synthetic_valid_dataset_path)
    ...
    for batch in tqdm.tqdm(synthetic_valid_dataloader, desc="computing metrics if prediction=target"):
        metrics = dm_codes.utils.compute_metrics(target=batch["target"], pred=batch["target"], text=batch["text"], prefix="perfect_network/")
    ...
    for batch in tqdm.tqdm(synthetic_valid_dataloader, desc="computing metrics if prediction=input"):
        ...
        metrics = dm_codes.utils.compute_metrics(target=batch["target"], pred=corrupt, text=batch["text"], prefix="copy_baseline/")
    ...
    for batch in tqdm.tqdm(real_valid_dataloader, desc="computing metrics if prediction=input in real dataset"):
        ...
        real_metrics = dm_codes.utils.compute_metrics(pred=image, text=batch["text"], prefix="copy_baseline_real/")
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        dirpath=f"{checkpoints_path}/{wandb_experiment.name}",
