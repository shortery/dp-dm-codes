...
limit = 128
synthetic_dataset = my_datasets.MyMapDatasetFromFolder(folder="./datasets/synthetic_valid_dataset", limit=limit)
dataloader_valid_synthetic = torch.utils.data.DataLoader(
    dataset=synthetic_dataset,
    batch_size=config["valid_batch_size"]
)
fst_synthetic_batch = next(iter(dataloader_valid_synthetic)) 
...
dataloader_valid_real = torch.utils.data.DataLoader(
    dataset=real_dataset,
    batch_size=config["valid_batch_size"]
)
fst_real_batch = next(iter(dataloader_valid_real)) 
...
perfect_metrics = []
for batch in tqdm.tqdm(dataloader_valid_synthetic, desc="computing metrics if prediction=target"):
...
baseline_metrics = []
for batch in tqdm.tqdm(dataloader_valid_synthetic, desc="computing metrics if prediction=input"):
...
baseline_metrics_real = []
for batch in tqdm.tqdm(dataloader_valid_real, desc="computing metrics if prediction=input in real dataset"):
    image = torchvision.transforms.functional.rgb_to_grayscale(batch["image"])
    metrics_real = my_utils.compute_metrics(pred=image, text=batch["text"], prefix="copy_baseline_real/")
    baseline_metrics_real.append(metrics_real)
baseline_metrics_real = pd.DataFrame(baseline_metrics_real).mean().to_dict()
print("if prediction=input in real dataset:")
print(baseline_metrics_real)
...
checkpoint_callback = pl.callbacks.ModelCheckpoint(
    ...
    filename="step={step}--corr_dec={synthetic_valid/correctly_decoded:.4f}",
...
wandb_logger.log_metrics(baseline_metrics_real)
wandb_logger.log_table(key="validation_characteristics", dataframe=pd.DataFrame([perfect_metrics | baseline_metrics | baseline_metrics_real]))

trainer.fit(
    model=autoencoder,
    train_dataloaders=dataloader_train,
    val_dataloaders=[dataloader_valid_synthetic, dataloader_valid_real],
)
